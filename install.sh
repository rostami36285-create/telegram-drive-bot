...
REPO_URL="https://github.com/Mysterious53/telegram-drive-bot.git"
BOT_USER="drivebot"
...
if [[ -d "$INSTALL_DIR/.git" ]]; then
  ...
  if ! git -C "$INSTALL_DIR" pull --ff-only 2>&1; then
    # اگر pull شکست خورد، مالکیت را برگردان و خارج شو
    chown -R "$BOT_USER:$BOT_USER" "$INSTALL_DIR" 2>/dev/null || true
    die "آپدیت کد شکست خورد.\n  ← مطمئن شوید اینترنت برقرار است و repo تغییر نکرده."
  fi
elif [[ -d "$INSTALL_DIR" ]]; then
  die "پوشه $INSTALL_DIR وجود دارد اما مخزن git نیست.\n  ← پوشه را پاک کنید: rm -rf $INSTALL_DIR"
else
...
# ── به‌روزرسانی .env ──────────────────────────────────────────
# در update، secret موجود را نگه می‌داریم تا webhook بدون disruption بماند
_existing_wh=$(grep "^WEBHOOK_SECRET=" "$ENV" 2>/dev/null | cut -d'=' -f2- || true)
if [[ -n "$_existing_wh" ]] && [[ ${#_existing_wh} -ge 16 ]]; then
  WH_SECRET="$_existing_wh"
else
